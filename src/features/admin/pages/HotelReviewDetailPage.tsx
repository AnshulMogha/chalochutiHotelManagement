...
function Container() {
  ...
  const handleApprove = async (remarks: string) => {
    ...
    try {
      if (isZonalHotelReviewerRole(user?.roles)) {
        await adminService.zonalApproveHotel(hotelId, { remarks });
      } else if (isQcReviewerRole(user?.roles)) {
        await adminService.qcApproveHotel(hotelId, { remarks });
      } else {
  ...
  const handleReject = async (remarks: string) => {
    ...
    try {
      if (isZonalHotelReviewerRole(user?.roles)) {
        await adminService.zonalRejectHotel(hotelId, { remarks });
      } else if (isQcReviewerRole(user?.roles)) {
        await adminService.qcRejectHotel(hotelId, { remarks });
      } else {
