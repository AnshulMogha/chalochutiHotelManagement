...
function normalizeReviewRemarks<T extends { qcRemarks?: ReviewRemark[] }>(
  ...
  return (items || []).map((item) => ({
    ...
    qcRemarks:
      item.qcRemarks ??
      ((item as { reviewRemarks?: ReviewRemark[] }).reviewRemarks || []),
  }));
}
...
export interface User {
  ...
  roles: string[];
  /** Some APIs may return `role` instead of `roles`. */
  role?: string[];
  stateIds?: number[];
  ...
  /** Present when API returns module grants (e.g. Hotel BD user-level permissions). */
...
export const adminService = {
  ...
  getHotelsForReviewPaginated: async (
    ...
    >(API_ENDPOINTS.ADMIN.GET_HOTELS_FOR_REVIEW, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
    };
  },
  getApprovedHotelsPaginated: async (
    ...
    >(API_ENDPOINTS.ADMIN.GET_APPROVED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getRejectedHotelsPaginated: async (
    ...
    >(API_ENDPOINTS.ADMIN.GET_REJECTED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getQcPendingHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<HotelReviewItem>>
    >(API_ENDPOINTS.ADMIN.GET_QC_PENDING_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getQcApprovedHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<ApprovedHotelItem>>
    >(API_ENDPOINTS.ADMIN.GET_QC_APPROVED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getQcRejectedHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<RejectedHotelItem>>
    >(API_ENDPOINTS.ADMIN.GET_QC_REJECTED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getZonalPendingHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<HotelReviewItem>>
    >(API_ENDPOINTS.ADMIN.GET_ZONAL_PENDING_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getZonalApprovedHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<ApprovedHotelItem>>
    >(API_ENDPOINTS.ADMIN.GET_ZONAL_APPROVED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getZonalRejectedHotelsPaginated: async (
    ...
    const response = await apiClient.get<
      ApiSuccessResponse<PaginatedResponse<RejectedHotelItem>>
    >(API_ENDPOINTS.ADMIN.GET_ZONAL_REJECTED_HOTELS, { params });
    const payload = response.data;
    return {
      ...payload,
      content: normalizeReviewRemarks(payload.content),
  ...
  getHotelsForReview: async (): Promise<HotelReviewItem[]> => {
    const response = await adminService.getHotelsForReviewPaginated();
    return response.content || [];
  },
  getApprovedHotels: async (): Promise<ApprovedHotelItem[]> => {
    const response = await adminService.getApprovedHotelsPaginated();
    return response.content || [];
  },
  getRejectedHotels: async (): Promise<RejectedHotelItem[]> => {
    const response = await adminService.getRejectedHotelsPaginated();
    return response.content || [];
  },
  getQcPendingHotels: async (): Promise<HotelReviewItem[]> => {
    const response = await adminService.getQcPendingHotelsPaginated();
    return response.content || [];
  },
  getQcApprovedHotels: async (): Promise<ApprovedHotelItem[]> => {
    const response = await adminService.getQcApprovedHotelsPaginated();
    return response.content || [];
  },
  getQcRejectedHotels: async (): Promise<RejectedHotelItem[]> => {
    const response = await adminService.getQcRejectedHotelsPaginated();
    return response.content || [];
  },
  getZonalPendingHotels: async (): Promise<HotelReviewItem[]> => {
    const response = await adminService.getZonalPendingHotelsPaginated();
    return response.content || [];
  },
  getZonalApprovedHotels: async (): Promise<ApprovedHotelItem[]> => {
    const response = await adminService.getZonalApprovedHotelsPaginated();
    return response.content || [];
  },
  getZonalRejectedHotels: async (): Promise<RejectedHotelItem[]> => {
    const response = await adminService.getZonalRejectedHotelsPaginated();
    return response.content || [];
  },
  approveHotel: async (
    hotelId: string,
  ...
  getUsers: async (params?: GetUsersParams): Promise<UsersResponse> => {
    ...
    return {
      ...payload,
      content: (payload.content || []).map((u) => ({
        ...u,
        roles: u.roles || u.role || [],
        stateIds: Array.isArray(u.stateIds)
          ? u.stateIds
          : Array.isArray(u.states)
            ? u.states.map((s) => s.id)
            : [],
      })),
    };
  ...
  /**
   * User-level module permissions (e.g. Hotel BD). API expects the user id in the path
   * (same `/hotel-access/{id}/permissions` route as hotel-access records use for staff).
   */
  assignUserPermissions: async (
    ...
    const response = await apiClient.put<ApiSuccessResponse<null>>(
      API_ENDPOINTS.HOTEL_ADMIN.ASSIGN_PERMISSIONS(userId),
      permissions,
    );
    return response.data;
  },
