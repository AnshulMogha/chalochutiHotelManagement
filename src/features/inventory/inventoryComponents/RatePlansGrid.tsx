...
interface RatePlansGridProps {
  ...
  /** From rates calendar root `isLinkEnable`; when false, no link buttons are shown. */
              ...
              >
                <div className={`flex items-start px-6 py-4 font-bold text-sm text-slate-900 border-r border-slate-200 bg-slate-50/60`}>
                  {isExpanded ? (
                    ...
                    <ChevronRight className="w-5 h-5 mr-3 mt-0.5 shrink-0 text-slate-600 transition-transform group-hover:translate-x-0.5" />
                  )}
                  <span className="min-w-0 flex-1 break-words text-left leading-snug group-hover:text-blue-600 transition-colors">
                    {room.roomName}
                  </span>
                </div>
