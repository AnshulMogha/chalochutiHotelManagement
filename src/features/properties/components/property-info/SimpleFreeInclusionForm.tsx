...
type SimpleFreeInclusionFormProps = {
  hotelId: string;
...
export function SimpleFreeInclusionForm({
  hotelId,
  inclusion,
  onBack,
  onSuccess,
  ...
  const canSubmit = datesValid && !saving;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEdit && editTarget) {
      ...
      try {
        ...
        showToast(`${inclusion.name || "Inclusion"} updated.`, "success");
        onSuccess();
      } catch (err) {
  ...
  const handleConfirmAssignments = async (
    ...
    try {
      ...
      showToast(`${inclusion.name || "Inclusion"} created.`, "success");
      setAssignOpen(false);
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-slate-50 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 p-4 sm:p-5">
        ...
        <InclusionSectionCard
          title="Offer"
          subtitle="This inclusion is free with no extra configuration fields."
          icon={Gift}
        ...
        >
          <div className="grid gap-2 text-sm sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
              ...
              <p className="font-medium text-slate-800">NONE</p>
            </div>
          </div>
        </InclusionSectionCard>
